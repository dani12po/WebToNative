...
function validateTheme(raw, fallback) {
  const layouts = new Set(['split', 'centered', 'split-reverse', 'sidebar', 'glass', 'marketplace', 'service', 'industrial', 'culinary']);
  const dashboardStyles = new Set(['commerce', 'operations', 'studio', 'executive', 'schedule', 'learning', 'service', 'industrial', 'culinary']);
  const loginStyles = new Set(['split', 'spotlight', 'editorial', 'showcase', 'minimal', 'marketplace', 'service', 'industrial', 'culinary']);
  return {
    id: 'ai-custom',
...
export async function promptAndGenerateAiTheme(projectName, profile, fallbackTheme) {
  ...
  const prompt = `Anda adalah art director SaaS Indonesia. Rancang sistem visual yang benar-benar sesuai dengan bisnis berikut, bukan tema generik.\nJudul proyek: ${projectName}\nJenis aplikasi: ${profile.name}\nKegunaan: ${profile.tagline}\n\nKembalikan JSON SAJA tanpa markdown dengan schema: {"name":"nama gaya singkat","primary":"#RRGGBB","secondary":"#RRGGBB","dark":"#RRGGBB","soft":"#RRGGBB","background":"#RRGGBB","layout":"split|centered|split-reverse|sidebar|glass|marketplace|service|industrial","dashboardStyle":"commerce|operations|studio|executive|schedule|learning|service|industrial","loginStyle":"split|spotlight|editorial|showcase|minimal|marketplace|service|industrial","fontFamily":"Inter|Manrope|DM Sans|Nunito Sans|Plus Jakarta Sans"}.\n\nArti layout: centered = navigasi horizontal/topbar untuk booking, kalender, atau layanan; split = sidebar ringkas untuk operasi; split-reverse = panel navigasi kanan untuk pendidikan/kreatif; sidebar = kontrol cepat untuk kasir/retail; glass = workspace ringan untuk layanan modern; marketplace = navigasi topbar, kartu kategori, dan grid konten untuk kursus, komunitas, katalog, atau membership; service = topbar bersih, kartu layanan, dan ruang putih profesional untuk klinik, salon, bengkel, konsultasi, atau jasa; industrial = topbar charcoal dengan aksen kuning untuk bengkel, kendaraan, gudang, parkir, atau operasional teknis.\nArti dashboardStyle: commerce = kartu kasir/retail; operations = tabel dan proses kerja padat; studio = bento kreatif dengan visual lembut; executive = ringkas, formal, banyak ruang kosong; schedule = agenda/booking terstruktur; learning = kartu konten dan progres untuk pendidikan/pelatihan; service = status layanan terpercaya, ringkasan pelanggan, dan kartu beraksen biru; industrial = informasi operasional yang tegas dengan status, tabel, dan aksen sinyal kuning.\nArti loginStyle: split = halaman merek dua panel; spotlight = kartu masuk terpusat dengan latar elegan; editorial = panel merek sempit dan form lega; showcase = halaman merek yang kuat dengan kartu mengambang; minimal = masuk yang sangat ringkas dan formal; marketplace = hero produk/edukasi dengan form rapi; service = hero biru-putih yang ramah dengan form masuk profesional; industrial = halaman akses gelap beraksen kuning yang tegas dan mudah dibaca. Pilih masing-masing satu yang paling relevan dengan bisnis. Jangan jadikan sidebar sebagai pilihan default: gunakan centered/topbar, split, split-reverse, glass, marketplace, service, atau industrial bila sama-sama relevan agar karakter tiap aplikasi bervariasi. Gunakan warna yang memiliki karakter industri, kontras aksesibel, dan hindari hijau default kecuali memang cocok.`;
  const culinaryGuidance = `\nPilihan tambahan yang valid: layout=culinary, dashboardStyle=culinary, loginStyle=culinary. Culinary adalah topbar hangat, kartu menu/layanan, dan nuansa heritage cokelat-krem-oranye untuk restoran, kafe, hotel, katering, atau event kuliner. Gunakan bila paling relevan.`;
  const response = await fetch(endpoint, {
    method: 'POST',
    headers: { Authorization: `Bearer ${config.api_key}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model: config.model, messages: [{ role: 'user', content: `${prompt}${culinaryGuidance}` }], temperature: 0.8 })
  });
...
export async function createMigrationTemplateBlueprint(projectName, profile, analysis) {
  ...
  const prompt = `Anda membuat blueprint desain reusable untuk generator migrasi GAS ke Next.js. Tidak boleh memberi kode. Jawab JSON saja dengan bentuk: {"id":"slug-lowercase","name":"maksimal 72 karakter","match":["6-12 kata kunci Indonesia"],"layout":"sidebar|topbar|rail","landing":"split|centered|editorial|marketplace|service|industrial","accent":"#RRGGBB","accent2":"#RRGGBB","description":"maksimal 180 karakter"}. Pilih desain SaaS profesional yang cocok khusus untuk aplikasi ini, bukan template bengkel/retail generik. Gunakan landing marketplace untuk pendidikan, kursus, komunitas, katalog, atau membership bila relevan. Gunakan landing service untuk klinik, salon, konsultasi, properti, dan jasa profesional: navigasi bersih, hero biru/teal yang ramah, kartu layanan, tanpa meniru merek lain. Gunakan landing industrial untuk bengkel, cuci kendaraan, parkir, gudang, logistik, atau operasional teknis: charcoal, aksen kuning sinyal, dan status kerja yang tegas. Hindari warna putih polos dan ikon emoji. Semua warna harus hex valid.\nNama proyek: ${projectName}\nJenis: ${profile.name}\nDeskripsi: ${profile.tagline}\nModul: ${JSON.stringify(modules)}\nArah AI: ${analysis?.uiDirection || 'Belum tersedia'}`;
  const culinaryBlueprintGuidance = `\nTambahan: landing=culinary valid untuk restoran, kafe, hotel, katering, atau event kuliner; gunakan warna cokelat, krem, dan aksen hangat tanpa meniru merek lain.`;
  const response = await fetch(endpoint, { method: 'POST', headers: { Authorization: `Bearer ${config.api_key}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: config.model, messages: [{ role: 'user', content: `${prompt}${culinaryBlueprintGuidance}` }], temperature: 0.35 }) });

export function getDatabaseGsTemplate(projectName, profile) {
  const modules = JSON.stringify(profile.modules).replace(/</g, '\\u003c');
  const safeProjectName = projectName.replace(/'/g, "\\'");
  return `/** ${safeProjectName} — database modular */
...
function DB_getSpreadsheet() {
  ...
  if (!id) { const ss = SpreadsheetApp.create('${safeProjectName} - Database'); props.setProperty('DB_SPREADSHEET_ID', ss.getId()); return ss; }
...
function DB_adminUpdateUser(payload) { AUTH_admin_(payload.adminUsername, payload.adminPassword); const targetUsername = String(payload.targetUsername || '').trim(), fullName = String(payload.fullName || '').trim(), role = String(payload.role || 'member'), newPassword = String(payload.newPassword || ''); if (!targetUsername || !fullName) throw new Error('Username target dan nama lengkap wajib diisi.'); if (role !== 'admin' && role !== 'member') throw new Error('Role tidak valid.'); if (newPassword && newPassword.length < 8) throw new Error('Password reset minimal 8 karakter.'); const sh = DB_sheet_(USER_SHEET), rows = sh.getDataRange().getValues(); for (let i = 1; i < rows.length; i++) { if (rows[i][1] === targetUsername) { sh.getRange(i + 1, 4).setValue(fullName); sh.getRange(i + 1, 5).setValue(role); if (newPassword) sh.getRange(i + 1, 3).setValue(DB_hash_(newPassword)); return { success:true, message:'Data pengguna berhasil diperbarui.' }; } } return { success:false, message:'Pengguna tidak ditemukan.' }; }
function DB_saveRecord(payload) { const module = DB_module_(payload.moduleId); if (module.adminOnly && payload.userRole !== 'admin') return { success:false, message:'Modul ini hanya dapat diakses Admin.' }; if (module.id === 'order' && module.name === 'Order Laundry') { const berat = Number(payload.berat || 0), harga = Number(payload.harga || 0), diskon = Number(payload.diskon || 0); payload.total = Math.max(0, berat * harga - diskon); } const sh = DB_sheet_(DB_sheetName_(module)); const values = module.fields.map(function(f) { return payload[f.key] || ''; }); sh.appendRow([DB_id_()].concat(values, [new Date(), payload.userName || ''])); return { success:true, message:module.name + ' berhasil disimpan.' }; }
function DB_getDashboard(moduleId, userRole) { const module = DB_module_(moduleId); if (module.adminOnly && userRole !== 'admin') return { success:false, message:'Modul ini hanya dapat diakses Admin.' }; const sh = DB_sheet_(DB_sheetName_(module)), rows = sh.getDataRange().getDisplayValues(), headers = rows.shift() || []; return { success:true, data:{ module:module.name, total:rows.length, headers:headers, records:rows.slice(-12).reverse() } }; }
function DB_getPaymentMethods() { const module = DB_module_('metodePembayaran'); const rows = DB_sheet_(DB_sheetName_(module)).getDataRange().getDisplayValues(); rows.shift(); return rows.slice(-20).reverse().map(function(row) { return { jenis: row[1], nama: row[2], penerima: row[3], nomor: row[4], qrisUrl: row[5], instruksi: row[6] }; }); }
`;
}

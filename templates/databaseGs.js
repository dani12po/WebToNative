...
function DB_saveRecord(payload) { const module = DB_module_(payload.moduleId); if (module.adminOnly && payload.userRole !== 'admin') return { success:false, message:'Modul ini hanya dapat diakses Admin.' }; const sh = DB_sheet_(DB_sheetName_(module)); const values = module.fields.map(function(f) { return payload[f.key] || ''; }); sh.appendRow([DB_id_()].concat(values, [new Date(), payload.userName || ''])); return { success:true, message:module.name + ' berhasil disimpan.' }; }
